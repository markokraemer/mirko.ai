...
EXCLUDED_DIRS = ['ui', 'cypress', 'node_modules', 'migrations', '.next', 'playwright-report', 'test-results', 'dist', 'build', 'coverage' ] 
EXCLUDED_EXT = ['.ico', '.svg', '.png', '.jpg','.jpeg', '.gif', '.bmp', '.tiff', '.webp']
...
def _should_exclude(root_path, path):
    ...
    for dir_name in EXCLUDED_DIRS:
        if os.path.commonpath([path, os.path.join(root_path, dir_name)]) == os.path.normpath(os.path.join(root_path, dir_name)):
            return True
    return False
...
def find_files(root_path, depth):
    ...
    for root, dirs, files in Path(root_path).walk():
        ...
        if len(current_depth) > depth:
            break
        for file in files:
            if file in EXCLUDED_FILES or any(file.endswith(ext) for ext in EXCLUDED_EXT):
                continue
            file_path = root / file
