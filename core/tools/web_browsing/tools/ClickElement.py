...
@dataclass
class ClickElementResult(ToolResult):
...
class ClickElement(Tool):
    ...
    def run(self) -> ClickElementResult:
        wd = get_web_driver()
...
DOM structure of the page.""".replace("\n", ""),
        ...
        while True:
            ...
            if "none" in message_text.lower():
                return self.fail_response("No element found that matches the description. To further analyze the page, use the AnalyzeContent tool.", 404)
            ...
            try:
                ...
                result = ClickElementResult(
                    success=True,
                    exit_code=0,
                    output=f"Clicked on element {number}. Text on clicked element: '{element_text}'.",
                    clicked_element_text=element_text,
                ...
                if error_count > 3:
                    return self.fail_response(f"Error clicking element. Error: {message} To further analyze the page, use the AnalyzeContent tool.", 500)
